...
document.querySelector("button[type='submit']")?.addEventListener("click", ()=>{
    // @ts-ignore
    const fileName : string = window.fileName
    ...
    let lyrics : (number | string) [][] = [];
    for (const lyricsElemElement of lyricsElem.children) {
        const timeElem = lyricsElemElement.querySelector("input[placeholder='TG']") as HTMLInputElement | null
        if (timeElem === null) {
            continue
        }

        const lyricElem = lyricsElemElement.querySelector("input[placeholder='Lời bài hát']") as HTMLInputElement | null
        if (lyricElem === null) {
            continue
        }

        lyrics.push(
            [timeToSecond(timeElem.value), lyricElem.value]
        )
    }

    let name = document.getElementById("songName") as HTMLInputElement | null;
    if (name === null) {
        throw Error("Wut")
    }

    let duration = document.getElementById("songDuration") as HTMLInputElement | null;
    if (duration === null) {
        throw Error("Wut")
    }
