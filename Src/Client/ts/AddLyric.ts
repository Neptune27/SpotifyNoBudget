...
const timeConverter = (val: string) => {
    if (!val.match(/:/g)) {
        val = "0:" + val;
    }
...
const AddLyricHandler = (lyrics : (number|string)[][] = []) => {
    ...
    const addItem = (TG = "", lyric = "") => {
        ...
        if (TG === "") {
            if (lyricSectionElem.lastElementChild !== null) {
                let val = lyricSectionElem.lastElementChild.querySelector("input[placeholder='TG']") as HTMLInputElement | null
                if (val !== null) {
                    TG = val.value
                }
            }
        ...
        const tgInp = div.querySelector("input[placeholder='TG']") as HTMLInputElement | null
        if (tgInp !== null) {
