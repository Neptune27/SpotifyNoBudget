        ...
        element.addEventListener('click', function (e) {
            ...
            fetch(`/Artist/getDataArtist/${artistId}`)
                ...
                .then(data => {
                    ...
                    artist = new Artist(temp.ID_USER, temp.NAME, temp.avatar, temp.MONTHLY_LISTENER, temp.verify);

                    albumsArtist = data.albums.map(element => new Album(element.ID_ALBUM, element.ALBUM_NAME,
                                                element.DESCRIPTIONS, element.ALBUM_IMG, element.LISTENERS));
                    songsArtist = data.songs.map(element => new Song(element.ID_MUSIC, element.MUSIC_NAME, null, element.TIME,
                                                    element.MUSIC_IMG, element.VIEW, element.ALBUM_NAME));
